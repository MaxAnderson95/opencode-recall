...
const layer = (hubConfig: Option.Option<PluginConfig.Hub>) =>
  Layer.mergeAll(Layer.succeed(PluginConfig.Service, { hub: Effect.succeed(hubConfig), hubSource: Effect.succeed("test"), summaryModel: Effect.succeed(PluginConfig.DEFAULT_SUMMARY_MODEL), file: "/cfg/recall.json", excludeDirectories: Effect.succeed([]) }), Source.fromDatabase(laptop.db))
